#!/usr/bin/env bash
# =============================================================================
# Passport Issuer VS — Local Setup
# =============================================================================
#
# This script sets up the Passport Issuer VS Agent locally (child service):
#   1. Deploys the VS Agent via Docker + ngrok
#   2. Sets up the veranad CLI account
#   3. Obtains a Service credential from organization
#   4. Obtains an ISSUER permission for the passport schema (VP flow)
...
VS_AGENT_IMAGE="${VS_AGENT_IMAGE:-veranalabs/vs-agent:v1.11.1-dev.2}"
VS_AGENT_CONTAINER_NAME="${VS_AGENT_CONTAINER_NAME:-passport}"
...
SERVICE_NAME="${SERVICE_NAME:-Passport Issuer}"
...
SERVICE_DESCRIPTION="${SERVICE_DESCRIPTION:-Passport credential issuer — NFC reading and liveness verification}"
SERVICE_LOGO_URL="${SERVICE_LOGO_URL:-https://2060.io/images/logo-squared.svg}"
...
sleep 1
ngrok http "$VS_AGENT_PUBLIC_PORT" --log=stdout > /tmp/ngrok-passport.log 2>&1 &
NGROK_PID=$!
sleep 5
...
# =============================================================================
# STEP 4: Obtain ISSUER permission for passport schema (VP flow)
# =============================================================================

log "Step 4: Obtain ISSUER permission for passport schema"

# Discover the passport schema from organization DID document
...
# Find the passport schema VTJSC in the org's DID document
log "Looking for passport schema VTJSC in organization..."
...
# Find the passport VTJSC (look for passport-jsc-vp in the service entries)
...
if [ -z "$CUSTOM_VP_URL" ]; then
  err "No passport schema VTJSC found in organization DID document"
  exit 1
fi
ok "Passport schema VTJSC VP: $CUSTOM_VP_URL"
...
if [ -z "$CUSTOM_SCHEMA_ID" ]; then
  err "Could not extract schema ID from passport VTJSC"
  exit 1
fi
ok "Passport schema ID: $CUSTOM_SCHEMA_ID"
...
if [ "$ENABLE_ANONCREDS" = "true" ]; then
  ...
  if [ "${EXISTING_ANONCREDS:-0}" -gt 0 ]; then
    ...
    # Find the VTJSC (json schema credential) for the passport schema
...
cat > "$OUTPUT_FILE" <<EOF
# Passport Issuer VS — Resource IDs
...
log "Passport Issuer VS setup complete!"
echo ""
...
echo "    docker compose -f passport/docker/docker-compose.yml up passport-issuer"
echo ""
echo "  To stop:"
